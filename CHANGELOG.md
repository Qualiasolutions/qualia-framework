...
- **Windows `pre-push.js` stamp-commit silently rolled back due to autocrlf.** On Windows installs with default `core.autocrlf=true`, the just-written LF-terminated `tracking.json` got normalized to CRLF in the git index, producing an empty diff against HEAD. The stamp-commit failed, the hook's rollback restored the original bytes, and `last_commit` never updated — ERP sync was silently stale on every Windows push. Surfaced by the newly-functional CI (Linux/macOS green, Windows red before the fix). Pass `-c core.autocrlf=false` on the stamp `git add` + `git commit` to keep the JSON as written regardless of local config.
